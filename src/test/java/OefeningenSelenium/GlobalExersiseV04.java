 ...
 */
public class GlobalExersiseV04 {
    public static void main(String[] args) throws IOException, ParseException {
        ...
        *-----------------------------------------------------------------------------------*/
        ...
        JSONObject TheJSONFile = (JSONObject) parser.parse(new FileReader("/Users/alexanderboffin/Documents/sandbox exercises/selenium/seleniumbasicexercises/Global exercise/docWebdriveGlobalS1xKENICHI.json"));
        ...
        //If we don't select anything for the basket steps 4b to 9 are futile (nothing to check out)
        //Set an element to standerd 0. If an element is placed into the basket the counter goes up. if it stays above 0=>go to the basket and beyond
        int needtoseethebasket=0;
        ...
        * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
        ...
        JSONArray  AccountAndAddress= (JSONArray) TheJSONFile.get("My Account en Adress");
        ...
        JSONArray  selectables= (JSONArray) TheJSONFile.get("Selectables");
        ...
        JSONArray  clickables= (JSONArray) TheJSONFile.get("Preference Click");
        ...
        * ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
        ...
        JSONArray IDAPW= (JSONArray)  TheJSONFile.get("LogInIdAndPassw");
        ...
        *+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ */
        ...
        JSONArray Searchfor= (JSONArray)  TheJSONFile.get("Search for");
        JSONArray SearchGoTO= (JSONArray)  TheJSONFile.get("Search GoTO");
        ...
        for (int Numie=0;Numie<Thesize;Numie++){
            ...
            //Extra clause=>if don't wan an animal this way after searching=>"Search GoTO" has as elemnt in it "STOP"
            String ToAnimalSearch=(String) SearchGoTO.get(Numie);
            if (!"STOP".equals(((String) SearchGoTO.get(Numie)).toUpperCase())) {
                ...
                needtoseethebasket=needtoseethebasket+1;//if animal loaded=>can do steps 4B-9
        ...
        * +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
        ...
        JSONArray Navogatefor= (JSONArray)  TheJSONFile.get("Navigate for");
        JSONArray NavigateGoTO= (JSONArray)  TheJSONFile.get("Natvigate GoTo");
        ...
        //??=>need to put an extra wait. Sometimes the test locks up afer last print function. It can take up to 5 tries before it locks up=>only thing I can think off
        //Or it can be that the element of the previous wait is much quicker to load than the banner????
        wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//a/img")));
        ...
        for (int Numbs=0;Numbs<Thesizenav;Numbs++){
            ...
            System.out.println(" Navigating throug links complete - moving on");


            String NAVigatableGO=(String) NavigateGoTO.get(Numbs);
            // Dirvergent path=>If no animal has to go into the basket (just looking no buying)
            if (!"STOP".equals(NAVigatableGO.toUpperCase())) {
        ...
        System.out.println("Gone over module of navigation (of the home page)");

        //Can place a split here=>can be that the person doesn't want to buy what he put in the basket=>would logg out then
        //check counter for first time=>if no animals added, no need to go further
        if (needtoseethebasket!=0) {


            /*EXTRA: REMOVE Animals form the basket the basket
            * +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
            System.out.println("Starting with removal");



            JSONArray animalstoremove= (JSONArray)  TheJSONFile.get("To remove");


            //Look how manny times we need to remove an animal
            int removables = animalstoremove.size();
            System.out.println("We need to remove "+removables+" Animals  in the basket");

            //for each animal: a loop
            for (int toremove=0;toremove<removables;toremove++){
                String animalneedsremoval= (String) animalstoremove.get(toremove);
                System.out.println("Are now removing: "+animalneedsremoval);

                //looking where on the page the link is
                int animalatlocation=2;//at site using the xpath the list of animalnames starts with //tr[2]/td[3]/a, need to change //tr[ ]
                while(!animalneedsremoval.equals(driver.findElement(By.xpath("//tr["+animalatlocation+"]/td[3]/a")).getText())){
                    animalatlocation=animalatlocation+1;
                }
                System.out.println("Found animal "+toremove+" at location: "+animalatlocation);

                //now know the location, whant to know how many animals are removed: to see if we need to go on (if no animals anymore)
                //tr[2]/td[6]/input
                needtoseethebasket=needtoseethebasket-(Integer.parseInt(driver.findElement(By.xpath("//tr["+animalatlocation+"]/td[6]/input")).getAttribute("value")));
                System.out.print("The number of animals in the basket is: "+needtoseethebasket);

                //remove the animal=>click on the removal button
                driver.findElement(By.xpath("//tr["+animalatlocation+"]/td/input")).click();

                //wait until page is loaded again
                wait.until(ExpectedConditions.visibilityOfElementLocated(By.cssSelector("span.title")));


            }
            System.out.println("Number of animals after removal: "+needtoseethebasket);

            //now if no animals anymore in basket=> skip te rest of steps
            if (needtoseethebasket!=0) {
            ...
            * +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
                ...
                JSONArray InBasket = (JSONArray) TheJSONFile.get("In Basket");
            ...
            /*Extra=Can be that we put animals in thebasket, but change our mind and just leave them (log out and schut down browser)
            +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
                if ("YES".equals(((String) TheJSONFile.get("To checkout")).toUpperCase())) {
        ...
        * +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++*/
        ...
        /*------------------------------------------------------------------------------------------------------------------------------------------
        cleanup=>log out and closing the window
        --------------------------------------------------------------------------------------------------------------------------------------------*/
